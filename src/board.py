class ConnectFour:
    ...
    def move(self, col: int, player: int) -> bool:
        
        if not (0 <= col < self.cols):
            raise ValueError(f'Poor column value: {col}')
        
        if player not in [ConnectFour.PLAYER_ONE, ConnectFour.PLAYER_TWO]:
            raise ValueError(f'Invalid player argument: {player}')

        for row in range(self.rows):
            if self.board[row][col] == ConnectFour.EMPTY_CELL:
                self.board[row][col] = player
                return self.check_connect_four((row, col), player)
